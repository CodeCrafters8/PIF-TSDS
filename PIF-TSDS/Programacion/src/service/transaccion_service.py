...
class TransaccionService:
    ...
    def comprar_accion(self, id_inversor, accion_id, cantidad):
        ...
        # Obtener la cantidad de acciones disponibles en el mercado
        cantidad_acciones_mercado = self.cotizacion_dao.obtener_cantidad_disponible(accion.id_accion)

        if cantidad_acciones_mercado is None:
            print("No se pudo obtener la cantidad de acciones disponibles en el mercado.")
            return False

        if cantidad_acciones_mercado < cantidad:
            # Imprimir la cantidad disponible en el mercado
            print(f"No hay suficientes acciones en el mercado para realizar la compra. Solo hay {cantidad_acciones_mercado} acciones disponibles.")
            return False
        ...
        self.user_dao.actualizar_saldo(user, total_a_restar, id_operacion=1)
        ...
        if self.portafolio_dao.existe_accion_en_portafolio(portafolio_id, accion_id):
            ...
            self.cotizacion_dao.actualizar_cantidad_disponible(cantidad, accion.id_accion, id_operacion=1)
        else:
    ...
    def vender_accion(self, id_inversor, accion_id, cantidad):
        ...
        if cantidad_disponible >= cantidad:
            ...
            # Actualizar el saldo del usuario en la base de datos
            self.user_dao.actualizar_saldo(user, total_a_sumar, id_operacion=2)

            # Actualizar la cantidad de acciones en el portafolio
            nueva_cantidad = cantidad_disponible - cantidad
            self.portafolio_dao.actualizar_accion(portafolio_id, accion_id, nueva_cantidad)
            self.cotizacion_dao.actualizar_cantidad_disponible(cantidad, accion_id, id_operacion=2)
            print("Venta exitosa.")
            return True
        else:
