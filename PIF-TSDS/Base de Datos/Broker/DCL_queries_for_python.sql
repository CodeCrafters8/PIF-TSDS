...
WHERE 
    p.id_inversor = (SELECT id_inversor FROM inversor WHERE email = %s)
GROUP BY 
    p.saldo_cuenta;
...
SELECT 
    ...
    (c.precio_compra_actual - (SELECT precio_operado FROM operacion o 
                               ...
                               AND o.inversor_id = (SELECT id_inversor FROM inversor WHERE email = %s) 
...
WHERE 
    o.inversor_id = (SELECT id_inversor FROM inversor WHERE email = %s) 
ORDER BY 
...
FROM 
    cotizacion 
WHERE 
    accion_id = (SELECT id_accion FROM acciones WHERE ticker = %s);
