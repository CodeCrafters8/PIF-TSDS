...
INSERT INTO perfil_inversor (tipo_inversor) VALUES 
('Conservador'),
('Moderado'),
('Agresivo');
...
INSERT INTO empresa (nombre, cuit, dommicilio, sector_id) VALUES 
('Apple Inc.', '30512345678', 'Avenida Siempreviva 742', 
	(SELECT id_sector FROM sector WHERE sector = 'Tecnología')),
('Tesla Inc.', '30698765432', 'Calle Salud 123', 
	(SELECT id_sector FROM sector WHERE sector = 'Energía')),
('JP Morgan Chase', '30456789123', 'Calle Energía 456', 
	(SELECT id_sector FROM sector WHERE sector = 'Finanzas')
);

-- Insertar datos de prueba en inversor
INSERT INTO inversor (cuil, nombre, apellido, email, contraseña, perfil_inversor_id)
VALUES 
...
INSERT INTO portafolio (saldo_cuenta, total_invertido, id_inversor)
VALUES 
(10000.00, 5000.00, (SELECT id_inversor FROM inversor WHERE email = 'juan.perez@example.com')),
(15000.00, 7000.00, (SELECT id_inversor FROM inversor WHERE email = 'ana.lopez@example.com')),
(20000.00, 10000.00, (SELECT id_inversor FROM inversor WHERE email = 'luis.martinez@example.com'));

-- Insertar datos de prueba en acciones 
INSERT INTO acciones (ticker, nombre, empresa_id)
VALUES 
('AAPL', 'Apple',
	(SELECT id_empresa FROM empresa WHERE nombre = 'Apple Inc.')),
('TSLA', 'Tesla', 
	(SELECT id_empresa FROM empresa WHERE nombre =  'Tesla Inc.')),
('JPM', 'JP Morgan', 
	(SELECT id_empresa FROM empresa WHERE nombre =  'JP Morgan Chase'));
